...
const (
	...
	// At the moment AllowedTopologies only supports
	// MatchLabelExpressions. Only iteration upto index 0
	// of []corev1.TopologySelectorTerm is possible.
	allowedTopologiesIndex = 0
...
func WithLocalPV() StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		s.ObjectMeta.Annotations[string(mconfig.CASTypeKey)] = "local"

		// Set the provisioner value for
		// openebs-localpv-provisioner PV controller
		s.Provisioner = provisionerName
...
func WithHostpath(hostpathDir string) StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		if scCASConfigStr, ok := s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)]; ok {
			...
			if !isCompatibleWithHostpath(scCASConfigStr) {
			...
			s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)] = scCASConfigStr + config

			return nil
		}

		// Add new CAS config Annotation key and value
...
func WithDevice() StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		if scCASConfigStr, ok := s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)]; ok {
			...
			if !isCompatibleWithDevice(scCASConfigStr) {
...
func WithAllowedTopologies(allowedTopologies map[string][]string) StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		for key, value := range allowedTopologies {
			s.AllowedTopologies[allowedTopologiesIndex].MatchLabelExpressions[i].Key = key
			s.AllowedTopologies[allowedTopologiesIndex].MatchLabelExpressions[i].Values = value
			i++
		}
		return nil
	}
...
func WithNodeAffinityLabel(nodeLabelKey string) StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		if scCASConfigStr, ok := s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)]; ok {
			// Check if the existing parameters are usable
			// with NodeAffnityLabel.
			// NodeAffinityLabel is only compatible with
			// Hostpath StorageType.
			if !isCompatibleWithNodeAffinityLabel(scCASConfigStr) {
...
func WithFSType(filesystem string) StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		if scCASConfigStr, ok := s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)]; ok {
			...
			// Device StorageType.
			if !isCompatibleWithFSType(scCASConfigStr) {
...
func WithBlockDeviceTag(bdLabelValue string) StorageClassOption {
	return func(s *storagev1.StorageClass) error {
		...
		if scCASConfigStr, ok := s.ObjectMeta.Annotations[string(mconfig.CASConfigKey)]; ok {
			...
			// Device StorageType.
			if !isCompatibleWithBlockDeviceTag(scCASConfigStr) {
				return errors.New("Failed to set BlockDeviceTag. " +
					"Invalid existing '" + string(mconfig.CASConfigKey) +
					"' parameters.")
			}
